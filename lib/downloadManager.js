...
function getPackageInfo(rootURL, packageName, options) {
    ...
    return new Promise(async (resolve, reject) => {
        console.info(
            moduleName +
                'downloadManager.getPackageInfo(): request to: ' +
                pkgList
        );
        ...
        try {
            ...
            console.error(
                moduleName +
                    'downloadManager.getPackageInfo: got.stream(): ' +
                    error
            );
...
async function downloadPkgFormPath(packagePath, options) {
    console.info(
        moduleName + 'downloadManager.downloadPkgFormPath(): start...'
    );
...
async function unzipPkg(zipFile, unzipDir, filterStr) {
    ...
    if (!fs.existsSync(zipFile)) {
        console.warn(
            moduleName + 'downloadManager.unzipPkg():No files to unzip.'
        );
        return;
    }
...
async function downloadSamsungCertfile(rootURL, destDir, options) {
    ...
    if (platform == 'macos') {
        ...
        await unzipPkgDir(
            tmpDir,
            tmpDir,
            'data/tools/certificate-manager/plugins'
        );
    }
