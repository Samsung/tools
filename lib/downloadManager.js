...
const platformToolDir = process.platform == 'win32' ? 'C:\\tizentv-tools' : path.resolve(process.env.HOME, 'tizentv-tools');
...
function getExtensionPkgInfo(extensionInfoUrl, packageName, options) {
    console.info(moduleName + 'downloadManager.getExtensionPkgInfo(): extensionInfoUrl = ' + extensionInfoUrl + ', packageName = ' +  packageName);
    return new Promise(async (resolve, reject) => {
        ...
        parser.parseString(data, function (err, result) {
            if (err) {
                console.error(moduleName + 'downloadManager.getExtensionPkgInfo():' + err);
                reject(err);
            } else {
...
async function downloadPkgFormPath(packagePath, options) {
    ...
    if (packagePath == undefined || packagePath == '') {
        console.error(moduleName + 'downloadManager.downloadPkgFormPath(): packagePath is invaild!');
        return;
    }
    makeFilePath(tmpDir);
    ...
    try {
        ...
        await pipeline(got.stream(packagePath, userAgent), fs.createWriteStream(tmpDir + path.sep + pkgName));
    } catch (error) {
...
async function unzipPkg(zipFile, unzipDir, filterStr) {
    console.info(moduleName + 'downloadManager.unzipPkg(): zipFile = ' + zipFile + ', unzipDir = ' + unzipDir + ', filterStr = ' + filterStr);
...
async function downloadSamsungCertfile(rootURL, destDir, options) {
    ...
    let samsungCertExtensionUrl = await getExtensionPkgInfo(rootURL + '/extension_info.xml', 'Samsung Certificate Extension', options);
    ...
    if (platform == 'macos') {
        await unzipPkgDir(tmpDir, tmpDir, 'data/tools/certificate-manager/Certificate-manager.app/Contents/Eclipse/plugins');
    } else {
